...
#ifndef _STRING_CONV_H
#define _STRING_CONV_H
...
public:
	 ...
	 */
	UniCodeToANSI_Convert(DWORD InCodePage = CP_ACP) :
		CodePage(InCodePage)
	{
	}
...
class UniCodeToOEM_Convert :
	public UniCodeToANSI_Convert
{
public:
	/**
	 * Sets the code page to OEM
	 */
	UniCodeToOEM_Convert() :
		UniCodeToANSI_Convert(CP_OEMCP)
	{
	}
};

/** Class that handles the ANSI to TCHAR conversion */
class ANSIToUniCode_Convert
{
public:
	/**
	 * Converts the string to the desired format. Allocates memory if the
	...
	{
#if _MSC_VER
		return (UINT)lstrlenW(Dest);
#else
		return (UINT)_tcslen(Dest);
#endif
	}
};

class UniCodeToUTF8_Convert
{
public:
	UniCodeToUTF8_Convert()
	{
	}
...
class UTF8ToUniCode_Convert
{
public:
	UTF8ToUniCode_Convert()
	{
	}
...
public:
	...
	inline ~TStringConversion()
	{
		if (ConvertedString != NULL)
		{
			...
			//@FY 2011.2.22 chenzhenqing revert
			//*ConvertedString = 0;
...
// Conversion typedefs
typedef TStringConversion<TCHAR,ANSICHAR,ANSIToUniCode_Convert> ANSIToUniCode;
typedef TStringConversion<ANSICHAR,TCHAR,UniCodeToANSI_Convert> UniCodeToANSI;
typedef TStringConversion<ANSICHAR, TCHAR, UniCodeToOEM_Convert> UniCodeToOEM;
typedef TStringConversion<ANSICHAR, TCHAR, UniCodeToUTF8_Convert> UniCodeToUTF8;
typedef TStringConversion<TCHAR, ANSICHAR, UTF8ToUniCode_Convert> UTF8ToUniCode;
