...
#ifdef DEBUG
CString g_baseUrl = (_T("http://127.0.0.1:8124"));
CString baseip = _T("192.168.122.24");
int baseport = 8124;
#else
CString g_baseUrl = (_T("http://gacenter.ztgame.com:8124"));
CString baseip = _T("http://gacenter.ztgame.com");
int baseport = 8124;
#endif
...
vector<CString> split_to_array(CString & src, CString seed)
{
...
map<CString, CString> split_to_map(const CString & src, CString head, CString val)
{
	map<CString, CString> strMap;
	CString tmp = src.GetString();
	//while (true)
	{
		CString n = tmp.SpanExcluding(L"=");
		if (n.GetLength() > 0)
		{
			tmp = tmp.Right(src.GetLength() - n.GetLength() - 1);
			strMap[n] = tmp;
		}
		else
		{
			//n = tmp.SpanExcluding(L"&");
		}
...
#ifdef DEBUG
	cmds[_T("notify")] = _T("1");
	cmds[_T("width")] = _T("272");
	cmds[_T("height")] = _T("192");
