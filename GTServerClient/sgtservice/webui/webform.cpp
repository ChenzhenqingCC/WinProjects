...
ULONG STDMETHODCALLTYPE WebForm::Release()
{
	...
	if (tmp == 0) {
		OutputDebugString(_T("WebForm::Release(): delete this"));
		delete this;
	}
...
LRESULT CALLBACK WebForm::WebformWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	if (msg == WM_NCCREATE) {
		...
		webf->setupOle();
		if (webf->ibrowser == 0) {
			MessageBox(NULL, _T("web->ibrowser is NULL"), _T("WM_CREATE"), MB_OK);
			delete webf;
			webf = NULL;
		} else {
			webf->AddRef();
...
void WebForm::create(HWND hWndParent, HINSTANCE hInstance, UINT id, bool showScrollbars)
{
	...
	if (!GetClassInfoEx(hInstance, WEBFORM_CLASS, &wcex)) {
		...
		if (!RegisterClassEx(&wcex)) {
			MessageBox(NULL, _T("Could not auto register the webform"), _T("WebForm::create"), MB_OK);
	...
	hWnd = CreateWindow(
		...
		0, 0, 100, 100, hWndParent, (HMENU)(LONG_PTR)id, NULL, (LPVOID)this);

}

HRESULT STDMETHODCALLTYPE WebForm::GetIDsOfNames(REFIID riid,
	LPOLESTR *rgszNames, UINT cNames, LCID lcid, DISPID *rgDispId)
{
	if (lstrcmpW(rgszNames[0], L"Call") == 0)
	{
		//网页调用window.external.Call时，会调用这个方法获取Call的ID
		*rgDispId = 9999;
	}
	return S_OK;
}
	...
	switch (dispIdMember) { // DWebBrowserEvents2
		case 9999:
		{
			//WORD a = wFlags;
					 ...
					 {
						 IDispatch * tag = (IDispatch*)pDispParams->rgvarg[0].pvarVal;
						 CComDispatchDriver dispdriver(tag);
						 CComVariant rt;
						 dispdriver.GetPropertyByName(L"param", &rt);
						 if (rt.vt == VT_BSTR)
						 {
							 std::string s = UniCodeToANSI(rt.bstrVal);
							 //CString s = COLE2T(rt.bstrVal);
							 if (s.find("quit") != -1)
							 {
								 //int a = 0;
								 //extern HWND tagWindow;
								// ShowWindow(GetParent(hWnd), SW_HIDE);
								 CloseWindow(GetParent(hWnd));
								//PostMessage(GetParent(hWnd), WM_HIDE, 0, 0);
			...
			//hr = pIDsp->Invoke(PropertyID, IID_NULL, GetUserDefaultLCID(), DISPATCH_PROPERTYGET, &dispparamsNoArgs, &Result, &Info, &
			//this->
			//pDispParams->rgvarg
		}
		break;
