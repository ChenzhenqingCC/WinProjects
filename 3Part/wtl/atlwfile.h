...
namespace ATL
{
	...
	public:
			...
			DWORD dwAttributes = FILE_ATTRIBUTE_NORMAL)
		{
			...
			Close();
			//check path exist
			if ((dwFlags == OPEN_ALWAYS)||(dwFlags == CREATE_ALWAYS))
			{
				if (!::PathFileExists(pstrFileName))
				{
					CString strPath = pstrFileName;
					int pos = strPath.ReverseFind(_T('\\'));
					strPath.Delete(pos,strPath.GetLength()-pos);
					SHCreateDirectoryEx(0,strPath,0);
				}
			}
		...
		BOOL IsOpen() const
		{
			return m_hFile != INVALID_HANDLE_VALUE && m_hFile != 0;
		}
		...
		{
			_ASSERTE(m_hFile!=INVALID_HANDLE_VALUE);
			DWORD dwsz =  ::GetFileSize(m_hFile, NULL);
			//if (dwsz == 0xFFFFFFFF)
			//{

			//}
			_ASSERTE(dwsz !=0xFFFFFFFF);
			return dwsz;
		}
		
		BOOL GetSize(LARGE_INTEGER* liFileSize) const 
		{
			ATLASSUME(m_hFile != NULL);

			return ::GetFileSizeEx(m_hFile, liFileSize);
		}

		static DWORD GetCompressSize(LPCTSTR strfile)
		{
			_ASSERTE(m_hFile!=INVALID_HANDLE_VALUE);
			return ::GetCompressedFileSize(strfile,0);
		}

		BOOL SetSize(DWORD dwNewLen)
		{
			_ASSERTE(m_hFile != INVALID_HANDLE_VALUE);
		...
		/**
		*@srcpath is with pattern in the end
		* default is *.*
		*@fn:BOOL (*pfn)(CFindFile&)
		*/
		template<class F> static BOOL WalkDirectory(LPCTSTR srcpath,F& fn)
		{
			BOOL result = TRUE;
			...
			}while (finder.FindNextFile() && result);
