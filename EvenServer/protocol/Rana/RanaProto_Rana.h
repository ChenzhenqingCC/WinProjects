...
{//协议定义
	...
	//游戏登录请求 RanaClient->RanaService
	static const WORD PL_LOGIN_REQ = 100;
	struct P_PL_Login_Req
	{
		Rana_UserName	username;	//用户名
		Rana_PassWord	password;	//密码
	};
...
{//封包函数
	...
	inline Zoic::Message & operator << (Zoic::Message &m, const P_PL_Login_Req & p)
	{
		m << p.username;
		m << p.password;
		return m;
	...
	inline Zoic::Message & operator >> (Zoic::Message &m, P_PL_Login_Req & p)
	{
		m >> p.username;
		m >> p.password;
		return m;
